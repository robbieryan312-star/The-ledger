...
import { isCandidatePick, findElectionCandidate } from '@/lib/electionCompare';
...
import { Candidate, Election, Issue } from '@/lib/types';
...
type ComparePolitician = Politician & { fecEntry?: FecFinanceEntry };

type CompareSide = {
  pick: string;
  politician?: ComparePolitician;
  candidate?: Candidate;
  displayName: string;
  lastName: string;
  issues: Issue[];
...
function resolveSide(
  pick: string,
  politicianById: Map<string, ComparePolitician>,
  election?: Election,
): CompareSide | null {
  ...
  const politician = politicianById.get(pick);
...
function isValidPick(
  ...
  return politicianById.has(pick);
}
...
function CompareContent({
  ...
  const featuredPols = politicians.filter((p) => p.recordType !== 'lightweight');
  ...
  useEffect(() => {
    ...
    if (paramA && isValidPick(paramA, politicianById)) setAPick(paramA);
    if (paramB && isValidPick(paramB, politicianById)) setBPick(paramB);
  }, [searchParams, politicianById]);

  const sideA = useMemo(() => resolveSide(aPick, politicianById, activeElection), [aPick, activeElection, politicianById]);
  const sideB = useMemo(() => resolveSide(bPick, politicianById, activeElection), [bPick, activeElection, politicianById]);

  const financeA = sideA?.politician?.campaignFinance ?? null;
  const financeB = sideB?.politician?.campaignFinance ?? null;
        ...
        <div className="space-y-5">
          <div className="rounded-xl border border-white/[0.08] p-4 flex items-start gap-3" style={cardStyle}>
            ...
            <div className="text-xs text-white/50 leading-relaxed">
              ...
              {(sideA?.politician?.fecEntry || sideB?.politician?.fecEntry) && (
          ...
          <div className="rounded-2xl border border-white/[0.08] overflow-hidden" style={cardStyle}>
            ...
            <div className="px-5">
              ...
              <CompareCell
                label={`Total Raised${sideA?.politician?.fecEntry || sideB?.politician?.fecEntry ? ' (FEC)' : sideA.isElectionOnly || sideB.isElectionOnly ? ' (race est.)' : ''}`}
                aVal={raisedA}
                bVal={raisedB}
              ...
              {sideA.politician && sideB.politician ? (
                <>
                  <CompareCell
                    ...
                    aVal={(financeA?.totalRaised ?? 0) > 0 ? Math.round(((financeA?.individualDonations ?? 0) / (financeA?.totalRaised ?? 1)) * 100) : 0}
                    bVal={(financeB?.totalRaised ?? 0) > 0 ? Math.round(((financeB?.individualDonations ?? 0) / (financeB?.totalRaised ?? 1)) * 100) : 0}
                    higherIsBetter={true}
                    format="percent"
                  />
...
export default function CompareContentWithSuspense(props: {
  politicians: ComparePolitician[];
  fecMeta: { asOf?: string; source: Source };
}) {
  return (
