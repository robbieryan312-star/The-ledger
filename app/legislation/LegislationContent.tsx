...
import type { LegislationSnapshotMeta } from '@/lib/data/legislation';
import type { LegislationBundleSlice } from '@/lib/types/snapshotTypes';
import {
  availableStagesFromBills,
  billStageLabel,
  filterBills,
  type BillSortKey,
  type ChamberFilter,
  type StageFilter,
} from '@/lib/legislationFilters';
...
function SponsorLine({
  bill,
  sponsorProfileByBioguide,
}: {
  bill: Bill;
  sponsorProfileByBioguide: Record<string, string>;
}) {
  const sponsor = bill.sponsor;
  if (!sponsor) {
    return <span className="text-gray-500 text-xs">Sponsor: no verified record available</span>;
  }
  const profileId = sponsor.bioguideId ? sponsorProfileByBioguide[sponsor.bioguideId] : undefined;
...
function BillCard({
  ...
  return (
    <div className="bg-[#0d1f35] rounded-xl p-4 border border-[#1e3a5f] hover:border-[#c8a951]/40 transition-colors">
      ...
      <SponsorLine bill={bill} sponsorProfileByBioguide={sponsorProfileByBioguide} />
...
function LegislationContent({
  ...
  allBills: Bill[];
  floridaLegislation: LegislationBundleSlice;
  sponsorProfileByBioguide: Record<string, string>;
}) {
  const searchParams = useSearchParams();
  const router = useRouter();
  const pathname = usePathname();
  const stages = useMemo(() => availableStagesFromBills(allBills), [allBills]);
        ...
        <>
          ...
          <div className="space-y-3">
            {filtered.length === 0 ? (
              ...
              filtered.map((bill) => (
                <BillCard key={bill.id} bill={bill} sponsorProfileByBioguide={sponsorProfileByBioguide} />
              ))
            )}
          </div>
        </>
...
export default function LegislationContentWithSuspense(props: {
  meta: LegislationSnapshotMeta;
  allBills: Bill[];
  floridaLegislation: LegislationBundleSlice;
  sponsorProfileByBioguide: Record<string, string>;
