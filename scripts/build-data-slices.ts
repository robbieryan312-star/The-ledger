...
/** Keep prior-good sections when a raw source file is missing on rebuild (core-rules §6). */
...
async function buildLegislationBundle() {
  ...
  if (!sections.length && !(existing?.sections?.length)) return;
