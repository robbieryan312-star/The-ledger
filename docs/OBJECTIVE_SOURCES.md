...
8. **CHANNEL PROOF BEFORE ROUTING (binding — owner 2026-07-22):** any NEW source/channel must
   ...
   channels stay out of SOURCE_LOOKUP / this matrix until proven (see Retired sources record below).
...
Retired sources — never request a key, never re-add: VoteSmart NPAT (API defunct/unobtainable),
ProPublica Congress API (retired upstream).
