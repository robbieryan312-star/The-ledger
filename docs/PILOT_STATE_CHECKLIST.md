...
| 8 | State political news (ingest only) | NewsAPI (`media`) | **Not on `/states/FL`** — slice `news-florida.json` exists for future/FL-politician use only | `ingest:news-fl` | **pipeline only** — not displayed on state page per owner spec |
