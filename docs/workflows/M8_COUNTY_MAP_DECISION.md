# M8 — Florida county-map drilldown: product decision brief

**Status:** ⛔ STOP for OWNER — prepare only; **build neither option** until owner picks.  
**Date:** 2026-07-20  
**Context:** `USAMap` ships with `countyByFips = {}` and `countiesByState = {}` (hardcoded empty). Clicking a county shows a pending/empty panel — **DEAD UI**. Checklist row 11 + Dual Reference Roadmap flag this as a product decision.
...
| Field | Detail |
|-------|--------|
| **What** | County map drilldown never shows real county officials/elections |
| **Where** | `components/map/USAMap.tsx` lines 19–20: empty `countyByFips` / `countiesByState` |
| **Evidence** | `FILE_INVENTORY_AUDIT.md` KEEP/DEAD note; `PILOT_STATE_CHECKLIST.md` row 11 **gap** |
| **UI effect** | County click → pending empty state; `OfficialCard` import is runtime-dead |

Existing FL data that Option A could wire (already on disk): ACS/BLS county economics in `data/florida/census/florida-counties-sample.json` (67 counties, `fetched-live`). That file is **demographics/labor**, not elected-officials roster — Option A still needs an officials source.
