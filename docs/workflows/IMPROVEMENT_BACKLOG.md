...
| IMP-ALLEGED-POLICY | alleged = contested person-claim only; banned surfaces omit; news listings keep media; verbatim+outcome on Controversies | P0 | cursor | **done** (PR #98 — STAGE THREE) | `ledger-data-policy.mdc` · `allegedPolicyGuard.test.ts` · `newsCorroboration.ts` |
| IMP-SAIDDID-NOMINEE | Said→Did nominee-name overlap — topic-bucket equality alone paired Marvit↔Westercamp | P0 | cursor | **done** (on alleged PR tip) | `sourceIntegrity.ts` · `saidDid.json` 14/15 · fixture `SAID_DID_KNOWN_BAD_NOMINEE_MISMATCH` |
