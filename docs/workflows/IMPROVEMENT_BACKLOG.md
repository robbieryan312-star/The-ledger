...
| IMP-VOTESMART-PURGE | Owner-overrule: zero live `votesmart` (history exempt only); no tombstone; generic `approvedSourceMatrixGuard` | P0 | cursor | **done** (tip `PENDING_TIP` — STAGE THREE) | `approvedSourceMatrixGuard.test.ts` · `lib/data/approvedSourceMatrix.ts` |
