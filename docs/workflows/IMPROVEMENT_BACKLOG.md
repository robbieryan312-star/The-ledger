...
| IMP-ALLEGED-POLICY | alleged = contested person-claim only; banned surfaces omit; news listings keep media; verbatim+outcome on Controversies; **repair before removal** | P0 | cursor | **done** (REJECT remedy — STAGE THREE on new tip) | `ledger-data-policy.mdc` · `allegedPolicyGuard.test.ts` · `newsCorroboration.ts` · S000033 controversies×2 · endorsements×3 |
...
| IMP-PROVENANCE-DEFAULTS | Delete fabricated `?? 'Journalism'|'Congressional Record'|'Recorded position'|'VoteSmart'` outlet defaults; omit when unresolved | P0 | cursor | **done** (STAGE THREE with alleged remedy) | `resolveRecordedOutlet.ts` · `provenanceOutletGuard.test.ts` · `saidDidVoteContext.ts` · `buildSaidDidDiffs.ts` |
