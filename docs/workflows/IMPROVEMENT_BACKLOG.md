...
| IMP-ALLEGED-POLICY | alleged = contested person-claim only; banned surfaces omit; news listings keep media; verbatim+outcome on Controversies; **repair before removal** | P0 | cursor | **done** (tip `18b5d3e` — STAGE THREE) | `ledger-data-policy.mdc` · `allegedPolicyGuard.test.ts` · `newsCorroboration.ts` · S000033 controversies×2 · endorsements×3 |
...
| IMP-PROVENANCE-DEFAULTS | Delete fabricated `??` outlet defaults; synthetic `DefunctSurveySource` in fixtures (no dead-source token); matrix criterion A reports file names | P0 | cursor | **done** (tip `18b5d3e` — STAGE THREE) | `resolveRecordedOutlet.ts` · `provenanceOutletGuard` · `approvedSourceMatrixGuard.test.ts` |
