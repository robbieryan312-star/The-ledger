...
| IMP-SAIDDID-ONESIDED-NOMINEE | Said→Did nominee-name overlap was too strict when only the confirmation vote title used formal nomination phrasing, silently dropping same-nominee informal CREC remarks | P1 | cursor | **done** (PR pending) | `sourceIntegrity.ts` · fixture `SAID_DID_KNOWN_GOOD_ONE_SIDED_NOMINEE` |
