...
| 2026-07-25 | M-CORPUS-DEPTH (S000033 votes/CREC/saidDid) | S000033 119-only → 117–119 full-depth | votes sync wall **1256.5s** (~2508 LIS roll calls, keyless Senate); build AFTER **47.674s** (prebuild **12.803s**); BEFORE build not measured (`/usr/bin/time` absent) | votes **201→2508** (s117/s118/s119); statements **11→11** (CREC blocked: `GOVINFO_API_KEY` empty — prior preserved, no wipe); saidDid **8→10**/15 (honest-gap note); profile dir **375099→3721954** bytes; endorsements **3→7** (kept prior 3; +Bowman/Bush/Lee/Mamdani — not AOC 2018: Sanders did not endorse pre-primary) | multi-congress Senate LIS under `--full-depth`; keyless Senate-only scoped path; apply script unions statements + rebuilds saidDid | `scripts/sync-votes-national.ts` · `scripts/apply-sanders-corpus-depth.ts` · `profiles/S000033/{votes,saidDid,endorsements}.json` · `/tmp/ledger-sync-votes-corpus.log` | CREC full-depth needs `GOVINFO_API_KEY`; saidDid 10/15 until more CREC Saids; platform-positions shape adapters (IMP-POS-SHAPE) |
