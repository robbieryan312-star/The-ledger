...
| 2026-07-26 | M-PROVENANCE-DEFAULTS | fabricated outlet `??` labels | n/a | zero matches for Journalism/Congressional Record/Recorded position/VoteSmart defaults | resolveRecordedOutlet or omit | `provenanceOutletGuard` · `resolveRecordedOutlet.ts` | never invent outlet labels |
