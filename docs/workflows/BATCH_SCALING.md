...
| 2026-07-26 | M-ALLEGED-POLICY | alleged = contested person-claim only | n/a (policy + data cleanup) | banned sections alleged=0; news listings not demoted; DSA endorsement omitted; incomplete controversies omitted; guard 7/7 | canonical def in ledger-data-policy; stop news tier demotion; omit not downgrade on banned surfaces | `allegedPolicyGuard` · `newsCorroboration.ts` · `approvedMediaQuotes.ts` · profile JSON cleanup | never park alleged on votes/Said/Said→Did/finance/… |
| 2026-07-26 | IMP-SAIDDID-NOMINEE | S000033 Said→Did nominee overlap | rebuild via `buildCrecSaidDidLinks` | saidDid 15→14/15 partial; Marvit↔Westercamp removed | nomination last-name intersection before topic-bucket equality | `sourceIntegrity.saidDidSubjectsOverlap` · fixture `SAID_DID_KNOWN_BAD_NOMINEE_MISMATCH` · validateSaidDidFile | find Marvit Did or leave honest-gap; never pair different nominees |
| 2026-07-26 | M-ALLEGED REJECT remedy | S000033 controversies/endorsements repair | n/a | controversies 1→2 (verbatim c2); endorsements 2→3 (DSA media) | repair-before-removal rule; no delete of sourced facts | `ledger-data-policy.mdc` · controversies.json · endorsements.json | never thin profiles by deleting sourced rows |
| 2026-07-26 | M-PROVENANCE-DEFAULTS | fabricated outlet `??` labels | n/a | zero matches for Journalism/Congressional Record/Recorded position defaults; fixtures use synthetic DefunctSurveySource | resolveRecordedOutlet or omit | `provenanceOutletGuard` · `resolveRecordedOutlet.ts` · matrix criterion A fix | never invent outlet labels; never put dead-source tokens in fixtures |
