...
## Guard suites (18 commands in prebuild + render-integrity postbuild — all must pass before commit)
...
| `test:no-unverified-official-data` | FL dashboard: no official/nonpartisan numbers without `fetchedLive:true` |
...
Prebuild runs all 18; postbuild runs `test:render-integrity` + client chunks. CI: `.github/workflows/guards.yml`.
