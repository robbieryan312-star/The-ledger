# Phase 17b — Batch Data Collection Workflow (owner-specified)
...
(binding rules), and the demo components (locked caps). Cursor executes; Claude Code reviews.
...
- Improvements found are written back into this file / the saved outline — the process itself
  accumulates, it does not reset each batch.
...
- **Exit condition:** continue until we can confirm **100% accuracy AND efficiency**. Accuracy and
  efficiency should measurably improve batch over batch.
...
| Next | ~50 | TBD | pending | pending | over-rejection test (false-negative guard) |
