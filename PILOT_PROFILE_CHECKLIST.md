...
| 6 | Platform / survey text | Official issues pages (`official`) FIRST → Ballotpedia (`nonpartisan`) → campaign site if qualifies | Track Record · Topic Record | `npm run sync:official-issues-positions -- --member S000033` | **done** — `sanders.senate.gov/issues/` accordion → 11 qualified stances (`profiles/S000033/positions.json`, tier `official`). Ballotpedia secondary (page poverty). `berniesanders.com/issues` redirects to homepage — does not qualify. |
...
| 8 | Said→Did pairing | topicPositions + `buildSaidDidDiffsFromTopicPositions` | Track Record · Said→Did panel | build-time | **partial** — **11**/15 CREC pairs with inline saidQuote+saidUrl; Marvit nomination Said unpaired and three broad-topic pairs removed (no matching Did in corpus after subject-overlap guards)|
...
| 11 | News mentions | Approved-outlet RSS → topic/tag RSS → GDELT → NewsAPI (`media`/`alleged`/`nonpartisan`) | News section | `npm run sync:news-rss -- --members S000033` | **partial** — **12**/15 subject/quote-qualified; listings stay `'media'` for approved outlets (M-ALLEGED-POLICY); `isVerified` false until 2+ corroborators; GDELT empty this window |
