...
| 6 | Platform / survey text | Ballotpedia (`nonpartisan`) | Track Record · Topic Record | `npm run sync:topic-positions` | **honest-gap** — `positions.json` `platformPositions` empty for all topics; manifest `positions: honest-gap` (was incorrectly `filled` until `ca24c8a` manifest parity sync) |
...
| 11 | News mentions | Approved-outlet RSS → topic/tag RSS → GDELT → NewsAPI (`media`/`alleged`/`nonpartisan`) | News section | `npm run sync:news-rss -- --members S000033` | **done** — **15**/15 (matcher D1 + corroboration D2; single-outlet media demoted to `'alleged'`; GDELT rate-limited this run — depth via Guardian/Hill topic feeds) |
